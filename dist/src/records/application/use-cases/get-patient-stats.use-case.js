"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.GetPatientStatsUseCase = void 0;
const common_1 = require("@nestjs/common");
const clinical_progress_calculator_1 = require("../../domain/services/clinical-progress-calculator");
const records_tokens_1 = require("../../records.tokens");
let GetPatientStatsUseCase = class GetPatientStatsUseCase {
    constructor(repository, clinicalProgressCalculator) {
        this.repository = repository;
        this.clinicalProgressCalculator = clinicalProgressCalculator;
    }
    async execute(user) {
        const records = await this.repository.findAllByPatient(user.userId, user.tenantId);
        return {
            patientId: user.userId,
            recordsCount: records.length,
            ...this.clinicalProgressCalculator.calculateStats(records),
        };
    }
};
exports.GetPatientStatsUseCase = GetPatientStatsUseCase;
exports.GetPatientStatsUseCase = GetPatientStatsUseCase = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, common_1.Inject)(records_tokens_1.RECORDS_REPOSITORY)),
    __metadata("design:paramtypes", [Object, clinical_progress_calculator_1.ClinicalProgressCalculator])
], GetPatientStatsUseCase);
//# sourceMappingURL=get-patient-stats.use-case.js.map